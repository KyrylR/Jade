...
pub trait Esp32s3Hardware {
    ...
    fn attestation_initialised(&self) -> bool {
        self.hardware_attestation_available()
    }
...
#[derive(Debug)]
pub struct Esp32s3DevicePlatform<H> {
    runtime_state: RuntimePlatformState,
    hardware: H,
}

impl<H> Esp32s3DevicePlatform<H> {
...
impl<H: Esp32s3Hardware> Platform for Esp32s3DevicePlatform<H> {
...
impl<H: Esp32s3Hardware> RuntimePlatformStateAccess for Esp32s3DevicePlatform<H> {
...
impl<H: Esp32s3Hardware> DevicePlatform for Esp32s3DevicePlatform<H> {
    fn manifest(&self) -> DeviceManifest {
        self.hardware.manifest()
    }
...
impl<H: Esp32s3Hardware> Esp32s3PlatformShim for Esp32s3DevicePlatform<H> {
    fn serial_send(&mut self, bytes: &[u8]) -> Result<(), DeviceBootFailure> {
    ...
    fn usb_send(&mut self, bytes: &[u8]) -> Result<(), DeviceBootFailure> {
        self.hardware.usb_send(bytes)
    }

    fn usb_recv(&mut self, out: &mut [u8]) -> Result<usize, DeviceBootFailure> {
        self.hardware.usb_recv(out)
    }
    ...
    fn hardware_attestation_available(&self) -> bool {
        self.hardware.hardware_attestation_available()
    }
    ...
    fn hardware_attestation_public_key_pem(
        ...
        self.hardware.hardware_attestation_public_key_pem(out)
    }
    ...
    fn hardware_attestation_ext_signature(
        ...
        self.hardware.hardware_attestation_ext_signature(out)
    }
    ...
    fn hardware_attestation_sign(
        ...
        self.hardware.hardware_attestation_sign(challenge, out)
    }
}
...
pub type Esp32s3OtaSession<W, V = jade_core::NoOtaUploadVerifier> = OtaWriteSession<W, V>;
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Esp32s3BoardLoopDecision {
    Continue,
    Stop,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Esp32s3BoardLoopStopReason {
    Hook,
    TickLimit,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct Esp32s3BoardLoopRun {
    pub booted: bool,
    pub ticks: u64,
    pub stop_reason: Esp32s3BoardLoopStopReason,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Esp32s3BoardLoopError {
    Boot(DeviceRuntimeError),
    Tick(FirmwareFrameError),
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Esp32s3BoardEntrypointError {
    App(Esp32s3BoardAppError),
    Loop(Esp32s3BoardLoopError),
}

pub trait Esp32s3BoardLoopHooks {
    fn next_inputs(&mut self) -> Esp32s3BoardLoopInputs {
        Esp32s3BoardLoopInputs::default()
    }

    fn on_boot(&mut self, _report: &jade_core::DeviceBootReport) -> Esp32s3BoardLoopDecision {
        Esp32s3BoardLoopDecision::Continue
    }

    fn on_boot_error(&mut self, _error: DeviceRuntimeError) -> Esp32s3BoardLoopDecision {
        Esp32s3BoardLoopDecision::Stop
    }

    fn on_tick(&mut self, _report: &Esp32s3BoardTickReport<'_>) -> Esp32s3BoardLoopDecision {
        Esp32s3BoardLoopDecision::Continue
    }

    fn on_tick_error(&mut self, _error: FirmwareFrameError) -> Esp32s3BoardLoopDecision {
        Esp32s3BoardLoopDecision::Stop
    }
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub enum Esp32s3OtaStartError<E, V = core::convert::Infallible> {
    BootRequired,
    Manifest(&'static str),
...
pub fn begin_ota_update_verified<W, V>(
    ...
    request: OtaRequest,
    verifier: V,
) -> Result<Esp32s3OtaSession<W, V>, Esp32s3OtaStartError<W::Error, V::Error>>
where
    W: OtaImageWriter,
    V: OtaUploadVerifier,
{
    ...
    manifest
        .validate_ota_request(&request)
        .map_err(Esp32s3OtaStartError::Partition)?;
    OtaWriteSession::begin_verified(writer, request, verifier).map_err(Esp32s3OtaStartError::Writer)
}
...
{
    let mut event_loop =
        Esp32s3BoardStreamLoop::new_from_storage(platform, storage_backend, storage)
            .map_err(Esp32s3BoardEntrypointError::App)?;
    event_loop
        .run_until_hook_stop(hooks)
        .map_err(Esp32s3BoardEntrypointError::Loop)
}
...
#[derive(Debug)]
pub struct Esp32s3BoardStreamLoop<'a, P, B> {
    app: Esp32s3BoardStreamApp<'a, P, B>,
    ticks: u64,
}

impl<'a, P, B> Esp32s3BoardStreamLoop<'a, P, B>
where
    P: Esp32s3PlatformShim + jade_emulator::RuntimePlatform,
    B: jade_storage::StorageBackend,
{
    pub fn new(app: Esp32s3BoardStreamApp<'a, P, B>) -> Self {
        Self { app, ticks: 0 }
    }
    ...
    pub fn app_mut(&mut self) -> &mut Esp32s3BoardStreamApp<'a, P, B> {
        &mut self.app
    }

    pub fn into_app(self) -> Esp32s3BoardStreamApp<'a, P, B> {
        self.app
    }
    ...
    pub fn run_once<H>(
        &mut self,
        hooks: &mut H,
    ) -> Result<Esp32s3BoardLoopDecision, Esp32s3BoardLoopError>
    where
        H: Esp32s3BoardLoopHooks,
    {
        if !self.app.is_booted() {
            let report = match self.app.boot() {
                Ok(report) => report,
                Err(error) => {
                    if hooks.on_boot_error(error) == Esp32s3BoardLoopDecision::Stop {
                        return Err(Esp32s3BoardLoopError::Boot(error));
                    }
                    return Ok(Esp32s3BoardLoopDecision::Continue);
                }
            };
            if hooks.on_boot(&report) == Esp32s3BoardLoopDecision::Stop {
                return Ok(Esp32s3BoardLoopDecision::Stop);
            }
        }
        ...
        match tick {
            ...
            Err(error) => {
                ...
                if decision == Esp32s3BoardLoopDecision::Stop {
                    Err(Esp32s3BoardLoopError::Tick(error))
                } else {
                    Ok(decision)
                }
            }
...
mod tests {
    ...
    impl Esp32s3Hardware for TestPlatform {
        fn manifest(&self) -> DeviceManifest {
            self.manifest
        }
    ...
    fn s3_device_platform_adapter_delegates_hardware_and_runtime_state() {
        ...
        assert_eq!(info.board_type, Cow::Borrowed("jade_v2"));
        ...
        platform.hardware_mut().usb_rx = Some(vec![4, 5]);
        platform.hardware_mut().ble_rx = Some(vec![6, 7]);
        platform.hardware_mut().camera_rx = Some(b"qr".to_vec());
        platform.hardware_mut().touch_rx = Some(TouchEvent::Press { x: 10, y: 20 });
        let mut out = [0u8; 8];
        assert_eq!(
            Esp32s3PlatformShim::serial_recv(&mut platform, &mut out).unwrap(),
            3
        );
        assert_eq!(&out[..3], &[1, 2, 3]);
        assert_eq!(
            Esp32s3PlatformShim::usb_recv(&mut platform, &mut out).unwrap(),
            2
        );
        assert_eq!(&out[..2], &[4, 5]);
        assert_eq!(
            Esp32s3PlatformShim::ble_recv(&mut platform, &mut out).unwrap(),
            2
        );
        assert_eq!(&out[..2], &[6, 7]);
        assert_eq!(
            Esp32s3PlatformShim::camera_qr_scan(&mut platform, &mut out).unwrap(),
            2
        );
        assert_eq!(&out[..2], b"qr");
        assert_eq!(
            Esp32s3PlatformShim::touch_poll(&mut platform).unwrap(),
            Some(TouchEvent::Press { x: 10, y: 20 })
        );

        Esp32s3PlatformShim::serial_send(&mut platform, b"serial").unwrap();
        Esp32s3PlatformShim::usb_send(&mut platform, b"usb").unwrap();
        Esp32s3PlatformShim::ble_send(&mut platform, b"ble").unwrap();
        assert_eq!(platform.hardware().serial_tx, vec![b"serial".to_vec()]);
        assert_eq!(platform.hardware().usb_tx, vec![b"usb".to_vec()]);
        assert_eq!(platform.hardware().ble_tx, vec![b"ble".to_vec()]);

        Esp32s3PlatformShim::display_status(&mut platform, DisplayStatus::Busy("busy")).unwrap();
        assert_eq!(
            Esp32s3PlatformShim::confirm_user(
                &mut platform,
        ...
        platform.hardware_mut().attestation_pubkey_pem = Some(b"pem".to_vec());
        platform.hardware_mut().attestation_ext_signature = Some(b"ext".to_vec());
        platform.hardware_mut().attestation_signature = Some(b"sig".to_vec());
        assert!(Esp32s3PlatformShim::hardware_attestation_available(
            &platform
        ));
        assert_eq!(
            Esp32s3PlatformShim::hardware_attestation_public_key_pem(&mut platform, &mut out)
                .unwrap(),
            3
        );
        assert_eq!(&out[..3], b"pem");
        assert_eq!(
            Esp32s3PlatformShim::hardware_attestation_ext_signature(&mut platform, &mut out)
                .unwrap(),
            3
        );
        assert_eq!(&out[..3], b"ext");
        assert_eq!(
            Esp32s3PlatformShim::hardware_attestation_sign(&mut platform, b"challenge", &mut out)
                .unwrap(),
            3
        );
        assert_eq!(&out[..3], b"sig");
        assert_eq!(
            platform.hardware().attestation_challenge,
    ...
    #[test]
    fn s3_device_platform_adapter_runs_board_stream_loop() {
        let mut platform = Esp32s3DevicePlatform::new(TestPlatform::new(JADE_V2_MANIFEST));
        platform.hardware_mut().usb_rx = Some(v1_request("u", "ping"));
        ...
        assert_eq!(app.runtime().platform().hardware().usb_tx.len(), 1);
    }
    ...
    #[derive(Debug)]
    struct S3LoopTestHooks {
        boots: usize,
        ticks: usize,
        stop_after: usize,
        last_transports: Esp32s3V1PollReport,
        last_touch: Option<TouchEvent>,
        last_confirmation: Option<UserConfirmationDecision>,
    }

    impl S3LoopTestHooks {
        fn new(stop_after: usize) -> Self {
            Self {
                boots: 0,
                ticks: 0,
                stop_after,
                last_transports: Esp32s3V1PollReport::default(),
                last_touch: None,
    ...
    impl Esp32s3BoardLoopHooks for S3LoopTestHooks {
        fn next_inputs(&mut self) -> Esp32s3BoardLoopInputs {
            Esp32s3BoardLoopInputs {
                display_status: Some(DisplayStatus::Busy("loop")),
                confirmation: Some(UserConfirmation::Export { label: "xpub" }),
            }
        }

        fn on_boot(&mut self, report: &jade_core::DeviceBootReport) -> Esp32s3BoardLoopDecision {
            self.boots += 1;
            assert_eq!(report.target, DeviceTarget::JadeV2);
            Esp32s3BoardLoopDecision::Continue
        }

        fn on_tick(&mut self, report: &Esp32s3BoardTickReport<'_>) -> Esp32s3BoardLoopDecision {
            self.ticks += 1;
            self.last_transports = report.transports;
            self.last_touch = report.touch;
    ...
    #[derive(Debug, Default)]
    struct S3BootRetryHooks {
        boot_errors: usize,
        boots: usize,
        ticks: usize,
    }

    impl Esp32s3BoardLoopHooks for S3BootRetryHooks {
        fn on_boot_error(&mut self, error: DeviceRuntimeError) -> Esp32s3BoardLoopDecision {
            assert_eq!(
                error,
    ...
    #[test]
    fn s3_board_stream_loop_boots_and_runs_until_hook_stop() {
        let mut serial_frames = vec![0; RX_BUFFER_BYTES];
        let mut usb_frames = vec![0; RX_BUFFER_BYTES];
        let mut ble_frames = vec![0; RX_BUFFER_BYTES];
        let mut qr = vec![0; QR_BUFFER_BYTES];
    ...
    #[test]
    fn s3_board_stream_loop_can_retry_after_boot_error_hook() {
        let mut serial_frames = vec![0; RX_BUFFER_BYTES];
        let mut usb_frames = vec![0; RX_BUFFER_BYTES];
        let mut ble_frames = vec![0; RX_BUFFER_BYTES];
        let mut qr = vec![0; QR_BUFFER_BYTES];
        ...
        platform.boot_report = jade_core::DeviceBootReport {
            entropy_ready: false,
            ..jade_core::DeviceBootReport::ok(DeviceTarget::JadeV2)
        };
        let app = Esp32s3BoardStreamApp::new(
            platform,
            jade_storage::MemoryStorage::new(),
            &mut serial_frames,
            &mut usb_frames,
        ...
        .unwrap();
        let mut event_loop = Esp32s3BoardStreamLoop::new(app);
        let mut hooks = S3BootRetryHooks::default();

        assert_eq!(
            event_loop.run_once(&mut hooks),
            Ok(Esp32s3BoardLoopDecision::Continue)
        );
        ...
        event_loop
            ...
            .runtime_mut()
            .platform_mut()
            .boot_report = jade_core::DeviceBootReport::ok(DeviceTarget::JadeV2);
        assert_eq!(
            event_loop.run_once(&mut hooks),
            Ok(Esp32s3BoardLoopDecision::Stop)
        );
        assert!(event_loop.app().is_booted());
        assert_eq!(hooks.boot_errors, 1);
        assert_eq!(hooks.boots, 1);
    ...
    #[test]
    fn s3_board_entrypoint_runs_from_nvs_and_static_storage() {
    ...
    #[test]
    fn s3_board_stream_loop_surfaces_tick_errors_without_sending() {
        let mut serial_frames = vec![0; RX_BUFFER_BYTES];
        let mut usb_frames = vec![0; RX_BUFFER_BYTES];
        let mut ble_frames = vec![0; RX_BUFFER_BYTES];
        let mut qr = vec![0; QR_BUFFER_BYTES];
        ...
        assert!(matches!(
            event_loop.run_once(&mut hooks),
            Err(Esp32s3BoardLoopError::Tick(FirmwareFrameError::Allocation(
                jade_core::AllocationFailure::ResponseTooLarge { limit: 4, .. }
            )))
        ));
        assert_eq!(hooks.boots, 1);
        assert_eq!(hooks.ticks, 0);
        assert_eq!(event_loop.app().runtime().platform().usb_tx.len(), 0);
    }
